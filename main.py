...
def data_changer(string):
  ...
  for i, char in enumerate(string):
    ...
    elif len(re.findall(regex_string, string[i-2:i+3])):
      returned.extend(["<|oper_start|>", char, "<|oper_end|>"])
...
def reverse_data_changer(string):
  ...
  for i, char in enumerate(string):
    if char == "<|oper_start|>" or char == "<|oper_end|>":
        continue
    else:
      returned.append(char)
...
model = torch.load("model3", weights_only=False)
m = model.to(device)
model.eval()
...
with open("test_questions", "r", encoding="utf-8") as file:
  ...
  for i, seq in enumerate(equations):
    ...
    if inputs[0] == "STOP":
      ...
      mod_input = data_changer("\n" + inputs[0])
      ans = int(inputs[1])
      context = torch.tensor((encode(mod_input), ), dtype=torch.long, device=device)
      output = reverse_data_changer(decode(m.generate(context, max_new_tokens=(len(inputs[0]) + len(inputs[1]) + 3))[0].tolist()))
